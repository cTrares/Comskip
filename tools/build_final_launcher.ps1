...
$buildRoot = Join-Path $repoRoot "_temp\final-launcher-build"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Final release regression tests failed." }
    & $Python -B -m PyInstaller --noconfirm --clean --onefile --name comskip-final `
    ...
    if ($LASTEXITCODE -ne 0) { throw "Final launcher build failed." }
    & (Join-Path $portableRoot "comskip-final.exe") --version
    if ($LASTEXITCODE -ne 0) { throw "Final EXE did not start." }
}
finally {
